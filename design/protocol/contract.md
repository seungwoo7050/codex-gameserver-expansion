...
- 데이터 저장: 현재 모든 사용자/세션/큐/결과/레이팅 데이터는 프로세스 메모리에만 존재하며 재시작 시 초기화된다.
...
- 큐 참가 → 매칭 → 세션 시작/종료 → 결과 저장: 한 세션 종료 시 결과는 1건만 기록된다.
...
## 현재 미구현/제한
- MariaDB/Redis 연동은 설정값만 존재하며 실제 저장은 모두 메모리에서 처리된다.
- 리더보드/결과/레이팅/큐 상태는 프로세스 재시작 시 사라진다.
